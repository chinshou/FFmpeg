...
static unsigned char* stream_read_line(FILE *s,unsigned char* mem, int max, int utf16) {
/*
  int len;
  ...
  ptr[0] = 0;
  return mem;*/
  return fgets(mem, max, s);
}

inline static int stream_seek(FILE *s,off_t pos){
/*
...
//      putchar('*');fflush(stdout);
  ...
  return fseek(s, pos, SEEK_SET);
}

/*
void stream_reset(FILE *s){
	/*
  if(s->eof){
    s->pos=0;
...
static FILE* open_stream(const char* filename,char** options, int* file_format){

   return fopen(filename, "r");
}

static void free_stream(FILE *s){
	fclose(s);
}
...
static subtitle *sub_read_line_sami(FILE* st, subtitle *current, int utf16) {
...
static subtitle *sub_read_line_microdvd(FILE *st,subtitle *current, int utf16) {
...
static subtitle *sub_read_line_mpl2(FILE *st,subtitle *current, int utf16) {
    char line[LINE_LEN+1];
    char line2[LINE_LEN+1];
    char *p, *next;
    int i;
...
static subtitle *sub_read_line_subrip(FILE* st, subtitle *current, int utf16) {
    char line[LINE_LEN+1];
	...
	for (current->lines=1; current->lines < SUB_MAX_TEXT; current->lines++) {
	    ...
	    av_strlcpy (current->text[current->lines-1], q, len);
...
static subtitle *sub_read_line_subviewer(FILE *st,subtitle *current, int utf16) {
    char line[LINE_LEN+1];
    int a1,a2,a3,a4,b1,b2,b3,b4;
    char *p=NULL;
    int i,len;
	...
	for (i=0; i<SUB_MAX_TEXT;) {
		...
		//av_strlcpy (current->text[i], line, len); current->text[i][len]='\0';
                for(; j<len; j++) {
...
static subtitle *sub_read_line_subviewer2(FILE *st,subtitle *current, int utf16) {
    char line[LINE_LEN+1];
    int a1,a2,a3,a4;
    char *p=NULL;
    int i,len;
...
static subtitle *sub_read_line_vplayer(FILE *st,subtitle *current, int utf16) {
	char line[LINE_LEN+1];
	int a1,a2,a3;
...
static subtitle *sub_read_line_rt(FILE *st,subtitle *current, int utf16) {
...
static subtitle *sub_read_line_ssa(FILE *st,subtitle *current, int utf16) {
/*
 ...
 */
	...
	current->end   = 360000*hour2 + 6000*min2 + 100*sec2 + hunsec2;
		...
		av_strlcpy (current->text[num], line2, tmp-line2);
 ...
 */
static subtitle *sub_read_line_pjs(FILE *st,subtitle *current, int utf16) {
    char line[LINE_LEN+1];
...
static subtitle *sub_read_line_mpsub(FILE *st, subtitle *current, int utf16) {
	char line[LINE_LEN+1];
	float a,b;
	int num=0;
	char *p, *q;
...
static subtitle *sub_read_line_aqt(FILE *st,subtitle *current, int utf16) {
    char line[LINE_LEN+1];
    char *next;
    int i;
...
static subtitle *sub_read_line_subrip09(FILE *st,subtitle *current, int utf16) {
    char line[LINE_LEN+1];
    int a1,a2,a3;
    char * next=NULL;
    int i,len;
...
static subtitle *sub_read_line_jacosub(FILE* st, subtitle * current, int utf16)
{
	...
	for (q = line1; (!eol(*p)) && (current->lines < SUB_MAX_TEXT); ++p) {
		...
		if (*(p + 1) == 'n') {
		    *q = '\0';
		    q = line1;
		    current->text[current->lines++] = av_strdup(line1);
		    ++p;
		    break;
		}
		if ((toupper(*(p + 1)) == 'C')
...
static int sub_autodetect (FILE* st, int *uses_time, int utf16) {
    char line[LINE_LEN+1];
    int i,j=0;
    char p;
  ...
  while (l > from) {
    ...
    if(log2vis) {
      len = fribidi_remove_bidi_marks (visual, len, NULL, NULL,
				       NULL);
      if((op = av_malloc((FFMAX(2*orig_len,2*len) + 1))) == NULL) {
	//mp_msg(MSGT_SUBREADER,MSGL_WARN,"SUB: error allocating mem.\n");
	l++;
	break;
      }
...
struct subreader {
    subtitle * (*read)(FILE *st,subtitle *dest,int utf16);
...
const char* guess_cp(FILE *st, const char *preferred_language, const char *fallback)
{
    size_t buflen;
    unsigned char *buffer;
    const char *detected_sub_cp = NULL;

    buffer = av_malloc(MAX_GUESS_BUFFER_SIZE);
...
sub_data* sub_read_file (char *filename, float fps) {
    ...
    sub_num=0;n_max=32;
    first=av_malloc(n_max*sizeof(subtitle));
    if(!first){
#ifdef CONFIG_ICONV
	  subcp_close();
    ...
    while(1){
        if(sub_num>=n_max){
            n_max+=16;
            first=av_realloc(first,n_max*sizeof(subtitle));
        }
#ifndef CONFIG_SORTSUB
	sub = &first[sub_num];
#endif
...
// here we manage overlapping subtitles
	...
	do {
		...
		// subtitles
		second = av_realloc(second, (sub_num + sub_to_add + 1) * sizeof(subtitle));
		for (j = 0; j <= sub_to_add; ++j) {
		    int ls;
	    ...
	    // subs.
	    second = av_realloc(second, (sub_num + 1) * sizeof(subtitle));
    ...
    subt_data = av_malloc(sizeof(sub_data));
    subt_data->filename = av_strdup(filename);
...
char** sub_filenames(const char* path, char *fname)
{
    ...
    f_dir = av_malloc(len);
    f_fname = av_malloc(len);
    f_fname_noext = av_malloc(len);
    f_fname_trim = av_malloc(len);

    tmp_fname_noext = av_malloc(len);
    tmp_fname_trim = av_malloc(len);
    tmp_fname_ext = av_malloc(len);

    tmpresult = av_malloc(len);

    result = av_malloc(sizeof(subfn)*MAX_SUBTITLE_FILES);
	...
	tmp_sub_id = av_malloc(strlen(dvdsub_lang)+1);
...
//			fprintf(stderr, "%s priority %d\n", tmpresult, prio);
			if ((f = fopen(tmpresult, "rt"))) {
			    ...
			    result[subcnt].fname = av_strdup(tmpresult);
			    subcnt++;
			}
		    }
    ...
    if (tmp_sub_id) av_free(tmp_sub_id);

    av_free(f_dir);
    av_free(f_fname);
    av_free(f_fname_noext);
    av_free(f_fname_trim);

    av_free(tmp_fname_noext);
    av_free(tmp_fname_trim);
    av_free(tmp_fname_ext);

    av_free(tmpresult);

    qsort(result, subcnt, sizeof(subfn), compare_sub_priority);

    result2 = av_malloc(sizeof(char*)*(subcnt+1));
...
void sub_free( sub_data * subd )
{
    ...
    for (i = 0; i < subd->sub_num; i++)
        for (j = 0; j < subd->subtitles[i].lines; j++)
            av_free( subd->subtitles[i].text[j] );
    av_free( subd->subtitles );
