...
static int aes_encrypt(struct AVAES* paes, uint8_t *buffer, int buffer_size) {

  int readed=0;
  uint8_t* p = buffer;
  int block_size= 256;
  
  //av_log(NULL, AV_LOG_ERROR, "aes encrypt buffer size [%d]\n", buffer_size);
  while (buffer_size>0){
    ...
    //0:encrypt 1:decrpt
    av_aes_crypt(paes, p, p, readed >> 4, NULL, 0);
    p+=readed;
  }
  return 0;
}
...
static void caesar_encrypt(char *data, int length, const char *key) {
...
static void put_amf_string(FLVContext *flv, AVIOContext *pb, const char *str)
{
...
static void write_metadata(AVFormatContext *s, unsigned int ts)
{
    ...
    put_amf_string(flv, pb, "onMetaData"); // 12 bytes
    ...
    if (write_duration_filesize) {
        put_amf_string(flv, pb, "duration");
    ...
    while ((tag = av_dict_iterate(s->metadata, tag))) {
        ...
        put_amf_string(flv, pb, tag->key);
        avio_w8(pb, AMF_DATA_TYPE_STRING);
        put_amf_string(flv, pb, tag->value);
        metadata_count++;
    }

    if (write_duration_filesize) {
        put_amf_string(flv, pb, "filesize");
...
static int flv_write_trailer(AVFormatContext *s)
{
    ...
    if (build_keyframes_idx) {
        ...
        put_amf_string(flv, pb, "filepositions");
...
static int flv_write_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (par->codec_type == AVMEDIA_TYPE_DATA ||
        ...
        if (par->codec_id == AV_CODEC_ID_TEXT) {
            // legacy FFmpeg magic?
            avio_w8(pb, AMF_DATA_TYPE_STRING);
            put_amf_string(flv, pb, "onTextData");
            avio_w8(pb, AMF_DATA_TYPE_MIXEDARRAY);
            avio_wb32(pb, 2);
            put_amf_string(flv, pb, "type");
            avio_w8(pb, AMF_DATA_TYPE_STRING);
            put_amf_string(flv, pb, "Text");
            put_amf_string(flv, pb, "text");
            avio_w8(pb, AMF_DATA_TYPE_STRING);
            put_amf_string(flv, pb, pkt->data);
            put_amf_string(flv, pb, "");
            avio_w8(pb, AMF_END_OF_OBJECT);
        } else {
        ...
        if (par->codec_id == AV_CODEC_ID_AAC || par->codec_id == AV_CODEC_ID_H264){
          ...
          aes_encrypt(flv->paes, data ? data : pkt->data, size);
        }
        avio_write(pb, data ? data : pkt->data, size);
...
static const AVOption options[] = {
    ...
    //{"key",    "key string with 32chars", OFFSET(key),      AV_OPT_TYPE_STRING, .flags = E},
    { NULL },
};

static const AVClass flv_muxer_class = {
    .class_name = "tle muxer",
    .item_name  = av_default_item_name,
    .option     = options,
    .version    = LIBAVUTIL_VERSION_INT,
};

const FFOutputFormat ff_tle_muxer = {
    .p.name         = "tle",
    .p.long_name    = NULL_IF_CONFIG_SMALL("TLE"),
    .p.mime_type    = "video/x-tle",
    .p.extensions   = "tle",
