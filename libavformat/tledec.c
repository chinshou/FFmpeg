...
static int aes_decrypt(struct AVAES* paes, uint8_t *buffer, int buffer_size) {
  ...
  while (buffer_size>0){
    ...
    //0:encrypt 1:decrpt
    av_aes_crypt(paes, p, p, readed >> 4, NULL, 1);
    p+=readed;
  }
  return 0;
}
...
static void caesar_decrypt(char *data, int length, const char *key) {
...
static int amf_get_string(FLVContext *flv, AVIOContext *ioc, char *buffer, int buffsize)
{
    int ret;
    ...
    //decrypt
    //aes_decrypt(flv->paes, buffer, length);
    caesar_decrypt(buffer, length, "tle_caesar_key");
...
static int parse_keyframes_index(AVFormatContext *s, AVIOContext *ioc, int64_t max_pos)
{
    ...
    while (avio_tell(ioc) < max_pos - 2 &&
           amf_get_string(flv, ioc, str_val, sizeof(str_val)) > 0) {
                            ...
                            int64_t max_pos, int depth)
{
    ...
    case AMF_DATA_TYPE_STRING:
        if (amf_get_string(flv, ioc, str_val, sizeof(str_val)) < 0) {
...
static int flv_read_metabody(AVFormatContext *s, int64_t next_pos)
{
    ...
    if (type != AMF_DATA_TYPE_STRING ||
        amf_get_string(flv, ioc, buffer, sizeof(buffer)) < 0)
        return TYPE_UNKNOWN;
...
#if 0
    //for ios and android 
    int out_len = AV_BASE64_DECODE_SIZE(strlen(flv->base_key));
    decoded_data = (char*)av_malloc(out_len);
    if (!decoded_data) {
      return -1;
    }

    // 解码
    decoded_len = av_base64_decode(decoded_data, flv->base_key, out_len);
    if (decoded_len < 0) {
       av_free(decoded_data);
       return -1;
    }    
    flv->key = decoded_data;
#else
    HANDLE hMapFile;
    LPCTSTR pBuf;
...
#define BUFFER_SIZE 33
    ...
    //1:decrypt
    av_aes_init(flv->paes, flv->key, 256, 1); 
                           ...
                           int64_t dts, int64_t next)
{
    ...
    while (array || (ret = amf_get_string(flv, pb, buf, sizeof(buf))) > 0) {
...
retry_duration:
    ...
    if (ret < 0)
        return ret;
    if (st->codecpar->codec_id == AV_CODEC_ID_H264 || st->codecpar->codec_id == AV_CODEC_ID_AAC){
      //decrypt aes
      aes_decrypt(flv->paes, pkt->data, size);
    }    
...
static const AVOption options[] = {
    ...
    {"basek",    "base64 key", OFFSET(base_key), AV_OPT_TYPE_STRING, .flags = VD},
    { NULL }
};
...
const AVInputFormat ff_tle_demuxer = {
    .name           = "tle",
    .long_name      = NULL_IF_CONFIG_SMALL("TLE"),
