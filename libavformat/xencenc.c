...
#define IS_WEBM(mkv) (CONFIG_WEBM_MUXER && CONFIG_ENCX_MUXER ? \
...
#endif
    default:
        if (CONFIG_ENCX_MUXER && par->codec_id == AV_CODEC_ID_PRORES &&
                           ...
                           int is_default)
{
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (CONFIG_ENCX_MUXER && !native_id &&
                               ...
                               const AVPacket *pkt)
{
    ...
    if (CONFIG_ENCX_MUXER && st->codecpar->codec_id == AV_CODEC_ID_AAC) {
        if (pkt->size > 2 && (AV_RB16(pkt->data) & 0xfff0) == 0xfff0)
            ret = ff_stream_add_bitstream_filter(st, "aac_adtstoasc", NULL);
    } else if (st->codecpar->codec_id == AV_CODEC_ID_VP9) {
        ret = ff_stream_add_bitstream_filter(st, "vp9_superframe", NULL);
    } else if (CONFIG_ENCX_MUXER &&
...
const FFOutputFormat ff_encx_muxer = {
    .p.name            = "encx",
    .p.long_name       = NULL_IF_CONFIG_SMALL("Encx"),
    .p.mime_type       = "video/x-encx",
    .p.extensions      = "xev",
...
const FFOutputFormat ff_encx_webm_muxer = {
    .p.name            = "encx_webm",
    .p.long_name       = NULL_IF_CONFIG_SMALL("EncxWebM"),
    .p.mime_type       = "video/x-encx-webm",
    .p.extensions      = "xwebm",
    .priv_data_size    = sizeof(MatroskaMuxContext),
...
const FFOutputFormat ff_encx_audio_muxer = {
    .p.name            = "encx",
    .p.long_name       = NULL_IF_CONFIG_SMALL("Encx Audio"),
    .p.mime_type       = "audio/x-encx",
    .p.extensions      = "xea",
    .priv_data_size    = sizeof(MatroskaMuxContext),
    .p.audio_codec     = CONFIG_LIBVORBIS_ENCODER ?
                         AV_CODEC_ID_VORBIS : AV_CODEC_ID_AC3,
